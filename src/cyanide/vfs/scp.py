...
class ScpHandler:
    """
    Simplified SCP protocol handler for framework file capture.
    ...
    async def _handle_copy_command(self, header_str: str, dest_base: str) -> int:
        ...
        try:
            ...
            self.framework.save_quarantine_file(filename, content, self.session_id, self.src_ip)
    ...
    def _handle_dir_command(self, header_str: str, current_base: str) -> str:
    ...
    def _handle_end_dir_command(self):
    ...
    def _handle_end_dir(self, current_base: str) -> Tuple[str, bool]:
        """Pop directory stack and check if protocol should end."""
        if self.dir_stack:
            current_base = self.dir_stack.pop()
        self._handle_end_dir_command()
    ...
    async def handle(self, command: str) -> int:
        ...
        self._send_ack()
        return await self._process_sink_protocol(path)

    async def _process_sink_protocol(self, path: str) -> int:
        """Handle receiving files and directories in sink mode."""
        current_base = path
        self.dir_stack = [path]
        ...
        while True:
            ...
            if header_str.startswith("C"):
                ...
                current_base = self._handle_dir_command(header_str, current_base)
            elif header_str.startswith("E"):
                current_base, should_break = self._handle_end_dir(current_base)
                if should_break:
                    break
