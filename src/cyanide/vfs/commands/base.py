...
class Command:
    ...
    def username(self) -> str:
        """Get current effective username from emulator."""
        return str(self.emulator.username)
    ...
    def get_ip_addr(self) -> str:
        """Get the simulated IP address of the framework."""
    ...
    def is_pkg_mgr_supported(self, mgr: str) -> bool:
        ...
        if config_mgrs:
            return self._is_supported_by_config(mgr, config_mgrs)
        return self._is_supported_by_profile(mgr)

    def _is_supported_by_config(self, mgr: str, config_mgrs: list[str]) -> bool:
        """Private helper for config-based manager support."""
        ...
        # Family checks
        if mgr in ["apt", "apt-get", "dpkg"] and any(x in config_mgrs for x in ["apt", "debian"]):
            return True
        if mgr in ["yum", "dnf", "rpm"] and any(
            x in config_mgrs for x in ["yum", "rhel", "centos"]
        ):
            return True
        return False

    def _is_supported_by_profile(self, mgr: str) -> bool:
        """Private helper for profile-based manager support."""
    ...
    async def auth_and_execute(
        ...
        if needs_root:
            # Automate escalation for framework data collection
            return await self.execute(args, input_data=input_data)
    ...
    async def _execute_subcommand(self, args: list[str]) -> tuple[str, str, int]:
