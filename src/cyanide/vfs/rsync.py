...
class RsyncHandler:
    ...
    def __init__(self, session: Any, process=None):
        ...
        if not self.fs:
            self.fs = self.framework.get_filesystem(session.src_ip)
    ...
    async def handle(self, command: str) -> int:
        ...
        ssh_cfg = self.framework.config.get("ssh", {})
    ...
    async def _handle_push(self, dest_path: str) -> int:
        """Attacker pushed files TO framework. Parse file list for intelligence."""
        ssh_cfg = self.framework.config.get("ssh", {})
        rsync_cfg = ssh_cfg.get("rsync", {})
    ...
    def _handle_pull(self, src_path: str) -> int:
        """Attacker pulls files FROM framework."""
        self._log_event(
            "rsync_denied",
