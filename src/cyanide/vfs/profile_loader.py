...
def _compute_hash(base_file: Path, static_file: Path, rootfs_dir: Optional[Path] = None) -> str:
    ...
    for f_path in [base_file, static_file]:
        if f_path.exists():
...
def _load_base_data(base_file: Path) -> tuple[dict, dict, list, dict, dict]:
    """Helper to load base.yaml data."""
    ...
    with open(base_file, "r") as f:
        ...
        return (
            data.get("metadata", {}),
            data.get("dynamic_files", {}),
            data.get("honeytokens", []),
            data.get("system_templates", {}),
            data.get("static_files", {}),
        )


def _prepare_base_overlay(dynamic_files: Dict[str, Any]) -> Dict[str, Any]:
    """Pre-calculate base directory overlay for dynamic files."""
    base_overlay = {}
    parents = {posixpath.dirname(p) for p in dynamic_files}
...
def _prepare_context(metadata: Dict[str, Any], system_templates: Dict[str, Any]) -> Context:
    """Prepare VFS context with defaults and metadata."""
    context_data = {
        "os_name": metadata.get("os_name", "Linux"),
        "kernel_version": metadata.get("kernel_version", "5.10.0-8-amd64"),
        "hostname": metadata.get("hostname", "localhost"),
        "arch": metadata.get("arch", "x86_64"),
    }
    context_data.update(metadata)
    return Context(**context_data, system_templates=system_templates)
...
def _parse_yaml_profile(base_file: Path, static_file: Path) -> Dict[str, Any]:
    ...
    metadata, dynamic_files, honeytokens, system_templates, static_files = _load_base_data(
        base_file
    )
    static_manifest: Dict[str, Any] = {}

    _parse_tree_folders(static_manifest, static_files.get("tree_folders"))
...
def _check_disk_cache(
    ...
    try:
        ...
        try:
            ...
            if (
                not db_hash
                or db_hash[0] != target_hash
                or not db_v
                or db_v[0] != str(CACHE_FORMAT_VERSION)
            ):
                return None

            logger.info(f"Profile '{profile_name}' loaded from SQLite cache.")
            meta, dyn, honey, sys_tmpl, _ = _load_base_data(base_file)

            shared_backend = None
            if compiled_db.exists():
                shared_backend = SqliteBackend(str(compiled_db))
                shared_backend.is_shared = True
            ...
            return {
                ...
                "metadata": meta,
                "context": _prepare_context(meta, sys_tmpl),
                "dynamic_files": dyn,
                "honeytokens": honey,
                "system_templates": sys_tmpl,
                "base_overlay": _prepare_base_overlay(dyn),
                "shared_backend": shared_backend,
            }
        finally:
            conn.close()
    except Exception as e:
        _handle_cache_error(profile_name, compiled_db, e)
    return None


def _handle_cache_error(profile_name: str, compiled_db: Path, e: Exception):
    """Handle and log cache-related errors."""
    if isinstance(e, sqlite3.DatabaseError):
        logger.warning(f"SQLite cache for '{profile_name}' is corrupted: {e}. Deleting it.")
        try:
            compiled_db.unlink()
        except OSError:
            pass
