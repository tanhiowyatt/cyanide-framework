...
class LibvirtPool:
    ...
    async def _healthcheck_loop(self):
        ...
        while True:
            ...
            async with self.lock:
                for vm_id, v in self.vms.items():
                    await self._check_vm_health(vm_id, v)

    async def _check_vm_health(self, vm_id: str, v: dict):
        """Check the health of a single VM and rebuild if necessary."""
        conn = self.conn
    ...
    async def _recycle_loop(self):
        ...
        while True:
            ...
            async with self.lock:
                now = time.time()
                for vm_id, v in self.vms.items():
                    await self._check_vm_recycle(vm_id, v, now)

    async def _check_vm_recycle(self, vm_id: str, v: dict, now: float):
        """Check if a single VM needs recycling."""
        conn = self.conn
        if v["state"] == "ready" and conn and (now - v["last_used"] > self.vm_unused_timeout):
