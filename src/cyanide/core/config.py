...
def load_config(path: Path = Path("config/cyanide.cfg")):
    ...
    config = {
        ...
        "quarantine_path": cfg.get("honeypot", "quarantine_path", fallback="var/lib/cyanide/quarantine"),
        "ssh": {
    ...
    if cfg.has_section("ml"):
        config["ml"] = {
            ...
            "model_path": cfg.get("ml", "model_path", fallback="src/cyanide/ml/cyanideML/cyanideML.pkl"),
            "online_learning": cfg.getboolean("ml", "online_learning", fallback=False)
        }

    if cfg.has_section("cleanup"):
        config["cleanup"] = {
            "enabled": cfg.getboolean("cleanup", "enabled", fallback=True),
            "interval": cfg.getint("cleanup", "interval", fallback=3600),
            "retention_days": cfg.getint("cleanup", "retention_days", fallback=7),
            "paths": cfg.get("cleanup", "paths", fallback="var/log/cyanide,var/lib/cyanide/quarantine").split(",")
        }
