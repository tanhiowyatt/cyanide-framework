...
async def test_handle_exec_session_rsync(server):
    ...
    sess = MagicMock()
    factory = MagicMock()
    factory.framework.config = server.config
...
async def test_ssh_session_exec_requested(ssh_session):
    ...
    ssh_session.framework.services.analytics = MagicMock(ml_enabled=True)
    ssh_session.framework._analyze_command = MagicMock()
...
def test_ssh_session_ended(ssh_session):
    ...
    ssh_session.framework.logger.log_event.assert_called()
