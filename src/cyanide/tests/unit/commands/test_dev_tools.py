...
async def test_visudo(shell, mock_fs):
    ...
    stdout, stderr, rc = await cmd.execute([])
    assert shell.pending_input_callback is not None

    await shell.execute("G")
    await shell.execute("o")
    await shell.execute("guest ALL=(ALL) NOPASSWD:ALL")
    await shell.execute("\x1b")
    await shell.execute(":")
    stdout, stderr, rc = await shell.execute("w")
    await shell.execute("q")
    await shell.execute("\n")

    assert shell.pending_input_callback is None
