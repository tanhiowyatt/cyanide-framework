...
def mock_session():
    session = MagicMock()
    session.framework.config = {"ssh": {"rsync": {"enabled": True, "allow_upload": True}}}
...
async def test_rsync_push_upload_disabled(mock_session):
    ...
    mock_session.framework.config["ssh"]["rsync"]["allow_upload"] = False
    handler = RsyncHandler(mock_session)
    mock_session.channel.read.side_effect = [struct.pack("<i", 31), b"\x00"]
    rc = await handler.handle("rsync --server . dest")
    assert rc == 13
    mock_session.framework.logger.log_event.assert_any_call("conn_test_conn", "rsync_denied", ANY)
...
async def test_rsync_file_list_parsing(mock_session):
    ...
    rc = await handler.handle("rsync --server . dest")
    assert rc == 13
    mock_session.framework.logger.log_event.assert_any_call("conn_test_conn", "rsync_filelist", ANY)
