...
def test_ssh_factory_init(mock_framework):
    factory = SSHServerFactory(mock_framework)
    assert factory.framework == mock_framework
    assert factory._max_auth_tries == 5
    assert len(factory.conn_id) == 8


@pytest.mark.asyncio
async def test_ssh_factory_connection_made_logging(mock_framework, tmp_path):
    mock_framework.logger.log_dir = str(tmp_path)
    factory = SSHServerFactory(mock_framework)
    mock_conn = MagicMock()
    ...
    mock_framework.logger.log_event.assert_any_call(
...
def test_ssh_factory_publickey_auth(mock_framework):
    factory = SSHServerFactory(mock_framework)
    ...
    mock_framework.logger.log_event.assert_called_with(
        "conn_" + factory.conn_id,
        "auth.publickey",
        {
            "username": "attacker",
