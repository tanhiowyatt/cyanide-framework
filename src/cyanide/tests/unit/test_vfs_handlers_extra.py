...
def mock_session():
    ...
    session.framework.config = {"ssh": {"rsync": {"enabled": True}}}
    session.framework.logger = MagicMock()
...
async def test_rsync_init_no_fs(mock_session):
    ...
    mock_session.framework.get_filesystem.return_value = "fallback_fs"
    handler = RsyncHandler(mock_session)
    assert handler.fs == "fallback_fs"
    mock_session.framework.get_filesystem.assert_called_with("1.2.3.4")
...
async def test_rsync_handle_disabled(mock_session):
    mock_session.framework.config["ssh"]["rsync"]["enabled"] = False
    handler = RsyncHandler(mock_session)
...
async def test_rsync_handle_push_error(mock_session):
    ...
    with patch.object(RsyncHandler, "_read_file_list", side_effect=ValueError("list error")):
        ...
        assert result == 13
        handler.framework.logger.log_event.assert_any_call("conn_test_id", "rsync_error", ANY)
...
async def test_sftp_init_fallback(mock_session):
    ...
    mock_chan.framework = mock_session.framework
...
async def test_sftp_handler_close_readonly(mock_session):
    ...
    setattr(
        ...
        "cyanide_factory",
        MagicMock(framework=mock_session.framework),
    )
    handler = CyanideSFTPHandler(mock_chan)
