...
async def test_permissions_user(mock_fs):
    ...
    # permissions are now universally granted to authenticated users
    assert shell.check_permission("/etc/shadow", "r") is True
...
def test_get_prompt(shell):
    """Test centralized prompt generation logic."""
    # Test root prompt (hostname from debian profile in mock_fs is debian-server)
    shell.username = "root"
    assert shell.get_prompt("root", "/root") == "root@debian-server:/root# "

    # Test home directory abbreviation
    shell.env["HOME"] = "/root"
    assert shell.get_prompt("root", "/root") == "root@debian-server:~# "

    # Test user prompt
    shell.username = "user"
    shell.env["HOME"] = "/home/user"
    assert shell.get_prompt("user", "/home/user") == "user@debian-server:~$ "
    assert shell.get_prompt("user", "/var/log") == "user@debian-server:/var/log$ "

    # Test pending input prompt
    shell.pending_input_prompt = "confirm> "
    assert shell.get_prompt("root", "/root") == "confirm> "
