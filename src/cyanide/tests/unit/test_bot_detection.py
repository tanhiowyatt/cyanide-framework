...
@pytest.mark.asyncio
async def test_bot_detection_by_timing(mock_framework):
    session = SSHSession(mock_framework, MagicMock(), "1.1.1.1", 1234, "test-id")
    ...
    args, kwargs = mock_framework._analyze_command.call_args
    assert kwargs["is_bot"] is True


@pytest.mark.asyncio
async def test_bot_detection_by_paste(mock_framework):
    session = SSHSession(mock_framework, MagicMock(), "1.1.1.1", 1234, "test-id")
    session.channel = MagicMock()
...
@pytest.mark.asyncio
async def test_human_typing_timing(mock_framework):
    session = SSHSession(mock_framework, MagicMock(), "1.1.1.1", 1234, "test-id")
    session.channel = MagicMock()
