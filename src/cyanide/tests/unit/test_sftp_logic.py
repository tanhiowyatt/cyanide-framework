...
def mock_chan():
    ...
    conn.cyanide_factory.framework.logger = MagicMock()
    return chan
...
async def test_sftp_handler_close_upload(sftp_handler, mock_chan):
    ...
    framework = mock_chan.get_connection().cyanide_factory.framework
    ...
    framework.save_quarantine_file.assert_called_once_with(
...
async def test_sftp_handler_permissions(sftp_handler, mock_chan):
    ...
    config = mock_chan.get_connection().cyanide_factory.framework.config
