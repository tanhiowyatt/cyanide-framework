...
def mock_session(mock_logger):
    ...
    session.framework.save_quarantine_file = MagicMock()
    ...
    conn.cyanide_factory.framework = session.framework
...
async def test_rsync_handshake_and_error(mock_session, mock_process):
    ...
    mock_session.framework.logger.log_event.assert_called()
...
async def test_sftp_upload_via_handler(mock_session):
    ...
    mock_session.framework.save_quarantine_file.assert_called_with(
