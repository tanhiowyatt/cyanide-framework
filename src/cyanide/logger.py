...
class CyanideLogger:
    def __init__(self, config, async_logger=None):
        ...
        self.tty_log_path = (self.log_dir / "cyanide-tty.json").resolve()
        self.tty_log = self._setup_logger("cyanide_tty", self.tty_log_path)
    ...
    def _get_target_logger_info(self, event_type):
        ...
        if event_type.startswith("tty.") or event_type in [
            "tty.input",
            "tty.out",
            "tty.in",
        ]:
            return self.tty_log, self.tty_log_path

        if event_type.startswith("editor.") or event_type in [
            "command.input",
            "auth",
            "file.quarantine",
            "client_fingerprint",
    ...
    def _mirror_to_session(self, session_id, event_type, line):
        ...
        try:
            ...
            if (
                event_type.startswith("ml_")
                or event_type == "ml_thought"
                or event_type.startswith("editor.")
            ):
                if self.async_logger:
    ...
    def stop(self):
        """Shutdown all loggers and plugins."""
        for plugin in self.plugins:
            try:
                plugin.stop()
            except Exception as e:
