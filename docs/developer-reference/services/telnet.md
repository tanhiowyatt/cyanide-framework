...
If the configuration instructs the framework to run as a `proxy`, the `CyanideServer` bypasses `handle_connection` entirely and forces the raw TCP stream through the `TCPProxy` into an underlying target, treating Telnet essentially as an unfiltered socket pipe.
