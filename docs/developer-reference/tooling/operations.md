...
This guide covers running, monitoring, and analyzing data from the Cyanide Framework.
...
### Prometheus (Metrics)
Standard system and framework metrics are exposed on port `9090` at `/metrics`. 
...
2. The Framework must have privileges to reach the `qemu:///system` URI (e.g., matching groups or socket permissions).
...
### Quarantine Service
Any file transferred to the framework by an attacker (via `wget`, `curl`, `scp`, `sftp`, or `rsync`) is automatically:
