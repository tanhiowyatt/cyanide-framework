...
The Detection Engine distinguishes Cyanide from primitive frameworks. It combines rigid heuristics with Machine Learning modeling to categorize obfuscated or complex, novel payloads that simple Regex rules would typically miss.
...
- Native rotation (`time` or `size`) ensures the framework disk never fills up, preventing denial of service by logging exhaustion.
