...
### Detection & Alerting:
- **Configurable Honeytokens**: Replaced hardcoded alerting paths with a modular lookup system (Global Config > OS Profile). This allows distribution-specific alerts (e.g., `/etc/pacman.conf` for Arch Linux) without changing core code. **Update (May 2026):** Hardcoded fallback defaults have been completely removed to ensure no-noise operation by default.
