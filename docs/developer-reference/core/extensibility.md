...
The SSH server mimics specific `kex`, `macs`, and `ciphers` based on the [Security Profiling](../services/ssh.md) documentation. These are essentially plugins for the SSH handshake, allowing the framework to appear as an old Cisco router, a modern Ubuntu server, or a custom IoT firmware depending on your research goals.
