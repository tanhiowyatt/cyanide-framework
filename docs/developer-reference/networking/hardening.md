...
The most critical aspect of framework security is isolation. A compromised framework must not become a pivot point for attacking your internal infrastructure.
...
- **Stealth Mode**: Ensure the framework host does not respond to ICMP (ping) on the management interface.

### Egress Filtering (Outbound)
Strictly limit the framework's ability to "call home" or scan other targets:
...
- **Rate Limiting**: Limit the number of outbound connections to prevent the framework from participating in DDoS attacks.
...
- **Writable Volumes**: The only areas where the framework can write data are explicitly mounted volumes:
  - `/app/configs` (for configuration overrides or dynamic profiles)
  - `/app/var/log/cyanide` (for framework logs and audit trails)
...
### Host Protection
- **Dedicated Host**: Never run a framework on a host that performs other critical functions.
...
### Remote Logging
- **Do not store logs exclusively on the framework**. If an attacker compromises the container, they will delete the audit trail.
...
- **Container Health Failures**: Indicates potential DoS or crash of the framework services.
...
- **SSH Credentials**: Rotate the "framework" user passwords frequently to keep the environment fresh for new attackers.
